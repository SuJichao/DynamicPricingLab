...
"""提价系数上限 1.5 —— 销售再快收舱幅度也不超过 1.5 """

SOLO_FLT_PRICE_MULTIPLIER_MIN = 0.9
"""降价系数下限 0.9 —— 销售再慢放舱幅度也不低于 0.9 """
...
SOLO_KNN_FEATURE_COLS = (
    'FLT_YEAR', 'FLT_MONTH', 'date_sin', 'date_cos', 'HOL_LAST', 'chunjie_sin', 'BKD_SK', 'PRICE_OTA',
)
"""独飞航线 KNN 特征列"""

SOLO_KNN_TARGET_COLS = (
    'SRS_ZL_LEFT', 'PJPJ_LEFT', 'EX_DIF', 'PJPJ_FINAL',
)
"""独飞航线 KNN 预测目标列"""
...
MP_MAX_WORKERS = 8
"""多进程最大工作进程数（不超过 CPU 核心数）"""

MP_MAX_TASKS_PER_CHILD = 10
"""每个子进程处理的最大任务数，超过后回收重建，防止 cx_Oracle 内存缓冲区累积"""

ENV_DISABLE_MULTIPROCESSING = 'DISABLE_MULTIPROCESSING'
"""环境变量名：设置任意值即禁用多进程，强制使用单进程模式"""
