...
def _fetch_previous_ota_prices():
    """获取上一采集时点的外放价格和订座人数。"""
    return get_data(
        f"SELECT EX_DIF AS EX_DIF_OLD,TIME_PT_OLD,FLT_DATE,CARRIER,FLT_NO,"
        f"FLT_SEGMENT,BKD_OLD,PRICE_OTA_OLD FROM {SOLO_PREVIOUS_PRICE_TABLE}"
    )
...
def _calc_booking_increment(df):
    ...
    df['BKD_INC'] = (
        (df['BKD'] - df['BKD_OLD'])
        / (df['TIME_PT'] + (df['EX_DIF_OLD'] - df['EX_DIF']) * 24 - df['TIME_PT_OLD'])
    )
    return df
...
def _persist_surge_records(df):
    """将触发突增的航班记录持久化到数据库，并删除旧的重复记录。"""
    surge_records = df[df['PRICE_INCREASE'] >= 1][
        ['CATCH_DATE', 'EX_DIF', 'FLT_DATE', 'TIME_PT', 'CARRIER',
         'FLT_NO', 'FLT_SEGMENT', 'AVG_FARE_SK', 'BKD_INC', 'UP_DATE']
    ].copy()
...
def _load_previous_surge_and_floor(df):
    ...
    surge_history = get_data(
        "SELECT CATCH_DATE,EX_DIF,FLT_DATE,CARRIER,FLT_NO,FLT_SEGMENT,ADVICE_PRICE,PID "
        "FROM BKD_SUDDEN_INCREASE_RECORD"
    )
    df = pd.merge(df, surge_history, how='left',
                  on=['CATCH_DATE', 'EX_DIF', 'FLT_DATE', 'CARRIER', 'FLT_NO', 'FLT_SEGMENT'])
    df['AVG_FARE_SK'] = np.where(
        df['ADVICE_PRICE'] > 0,
        np.maximum(df['ADVICE_PRICE'], df['AVG_FARE_SK']),
        df['AVG_FARE_SK']
    )
    return df
